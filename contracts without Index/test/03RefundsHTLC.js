...
/// GREAT NUMBER OF TRANSACTIONS
  ...
  for (let i = 0; i < drexSigners.length-1; i++) {
    ...
    tradesToPushAlien = [];
      ...
      for (let i = 0; i < drexSigners.length; i++) {
        ...
        const _drexHashFields = temp[3];
        const _alienAliceAddress = temp[4];
        const _alienMikeAddress = temp[5];
        const _alienTokencontract = temp[6];
        const _alienAmount = temp[8];
        const _hashedSecret = temp[9];
        const _drexPreviousId = temp[11][4];
        const _drexTxIndex = temp[11][3];
        ...
        tradesToPushAlien.push([_drexHashFields,_alienAliceAddress,
          ...
          _hashedSecret,_drexPreviousId,_drexTxIndex,_drexTxId]);};
        }
     ...
     txIdsMikeRevert = [];
      ...
      for (let i=0; i<tradesToPushAlien.length; i++) {
        ...
        if (txIdsAlienPushed.indexOf(tradesToPushAlien[i][8])==-1) {
          ...
          //// PROBLEM IS HERE
          //// PROBLEM IS HERE
          if (tradesToPushAlien[i][7]>=tradesToPushAlien[i][9]) {
        //// PROBLEM IS HERE
          //// PROBLEM IS HERE
          //// PROBLEM IS HERE
          //// PROBLEM IS HERE
          ...
          //// PROBLEM IS HERE
          //// PROBLEM IS HERE
          //// PROBLEM IS HERE // not expired transactions
  ...
  // array with slots to be searched
    ...
    // half of the trades will not be completed by Alice because
    // because we want to test if she can get her refund in Alien Chain after time expiration
      for (let i=0; i<20; i++) { // there are 40 trades to confirm in Drex Chain
                                // we are settling only half to Alice get her refund in Alien Chain
  ...
  console.log ("** End  **");
          ...
          it("Alice is refunded because she does not settle the Tx in Drex",
            async function() {
      ...
      console.log ("** Starting Alice to get her refund for unsettled transactions **");
      console.log ("** Starting Alice to get her refund for unsettled transactions **");
     ...
     // await time.increase(alienExpiration*timeSlotDrex); //
      ...
      for (let i=20 ; i< TxValidation.length ; i++) {
        if (i==20) await time.increase(alienExpiration*timeSlotDrex); //
      expect (await alienListrack.connect(alfredAlien)
      .aliceGetRefund(TxValidation[i][0])) // 
       .to.emit(alienListrack,"AlienRefunded");
      }
     // }
