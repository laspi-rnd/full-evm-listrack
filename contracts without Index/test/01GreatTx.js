...
/// GREAT NUMBER OF TRANSACTIONS
  ...
  // array with slots to be searched
    ...
    // below array contain the required data for alien tx validation
      ...
      // the array below contain all slots that require validation
    ...
    /// BELOW IS THE MANUAL SETTLEMENT BY ALICE
     /// BELOW IS THE MANUAL SETTLEMENT BY ALICE
      /// BELOW IS THE MANUAL SETTLEMENT BY ALICE

    return {TxValidation,Listrack,merkleContract,alfred,oliver};

}

it("Alice manually Settles Tx using Merkle Proof",
        async function() {


    const {TxValidation,Listrack,merkleContract,alfred,oliver} = await loadFixture (InitialSetupFixture);


    console.log ("** Tx can now be manually settled by Alice **");
    ...
    console.log ("** ANOTHER SETTLEMENT : AUTOMATIC BY MERKLE CONTRACTS **");
    ...
    /// MERKLE CONTRACT REACHED CONSENSUS ON ALIEN CHAIN
     ...
     it("Merkle Contracts automatically Settle Transactions in Listrack",
        async function() { 
